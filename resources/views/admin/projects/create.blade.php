...
@section('title', 'Add Project - CodeFlow')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-[#0b132b]/40 backdrop-blur border border-slate-800/80 rounded-3xl p-8 shadow-xl" 
        ...
        <form action="{{ route('admin.projects.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select id="category" name="category" required
                        ...
                        <option value="Custom Systems" {{ old('category') == 'Custom Systems' ? 'selected' : '' }}>Custom System</option>
                        <option value="Mobile Apps" {{ old('category') == 'Mobile Apps' ? 'selected' : '' }}>Mobile Application</option>
            ...
            <div class="space-y-2">
                ...
                <div class="flex flex-col md:flex-row gap-6 items-start">
                    ...
                    <div class="w-full md:w-1/2 relative border-2 border-dashed border-slate-800 hover:border-cyan-500/50 rounded-2xl transition-colors bg-slate-950/30 p-6 flex flex-col items-center justify-center text-center group cursor-pointer">
                        <input type="file" name="thumbnail" id="thumbnail" required accept="image/*" @change="previewImage"
                        ...
                        <span class="text-sm font-semibold text-slate-300">Click to upload thumbnail</span>
                        <span class="text-xs text-slate-500 mt-1">PNG, JPG, WEBP up to 4MB</span>
                    </div>
                    ...
                    <div class="w-full md:w-1/2">
                        <div class="w-full h-36 rounded-2xl border border-slate-800 bg-slate-950/60 overflow-hidden flex items-center justify-center text-slate-500 relative">
                            ...
                            <template x-if="!imageUrl">
                                <span class="text-xs">No image selected. Preview will appear here.</span>
                            </template>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-2 pt-4 border-t border-slate-800/40">
                ...
                <div class="relative border-2 border-dashed border-slate-800 hover:border-cyan-500/50 rounded-2xl transition-colors bg-slate-950/30 p-6 flex flex-col items-center justify-center text-center group cursor-pointer">
                    <input type="file" name="images[]" multiple accept="image/*"
                           class="absolute inset-0 opacity-0 cursor-pointer z-10">
                    <svg class="w-10 h-10 text-slate-500 group-hover:text-cyan-400 transition-colors mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    <span class="text-sm font-semibold text-slate-300">Click to upload multiple slider screenshots</span>
                    <span class="text-xs text-slate-500 mt-1">Select multiple PNG, JPG, WEBP images</span>
                </div>
                @error('images')
                    <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="space-y-2">
                ...
                <textarea id="description" name="description" rows="5" required
                          ...
                          placeholder="Provide a detailed description of the project features, architecture, and solutions crafted...">{{ old('description') }}</textarea>
                @error('description')
                    <p class="text-xs text-rose-400 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="space-y-2">
                ...
                <input type="text" id="tech_stack" name="tech_stack" x-model="techInput" value="{{ old('tech_stack') }}" required
                       class="w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-2xl text-white placeholder-slate-600 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/20 transition-all"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <input type="url" id="live_demo" name="live_demo" value="{{ old('live_demo') }}"
                           class="w-full px-4 py-3 bg-slate-950/60 border border-slate-800 rounded-2xl text-white placeholder-slate-600 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/20 transition-all"
                        ...
                        <div class="relative bg-slate-950/40 border border-slate-800 rounded-2xl p-6 space-y-4">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <input type="text" x-model="plan.name" required
                                           class="w-full px-3 py-2 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-650 focus:outline-none focus:border-cyan-500 transition-all"
                                           placeholder="e.g. Premium SaaS">
                                </div>

                                <!-- Plan Price -->
                                <div class="space-y-1">
                                    <label class="text-[10px] font-semibold text-slate-450 uppercase tracking-wider block">Price</label>
                                ...
                                <!-- Billing Period -->
                                <div class="space-y-1">
                                    <label class="text-[10px] font-semibold text-slate-450 uppercase tracking-wider block">Billing Period</label>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 items-center">
                                ...
                                <div class="md:col-span-2 space-y-1">
                                    <label class="text-[10px] font-semibold text-slate-450 uppercase tracking-wider block">Plan Features (Comma separated)</label>
